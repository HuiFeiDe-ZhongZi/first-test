...
int main()
{
	...
	int readycode; // 就绪数量
	struct pollfd clientfd_array[4096];  // poll模型监听集合

	// 初始化数组
	for(int i = 0;i < 4096; i++)
	{
		clientfd_array[i].fd = -1; // 所有监听位都为非监听
	}
	
	clientfd_array[0].fd =serverfd;
	clientfd_array[0].events = POLLIN;

	printf("poll server version 1.0 Accepting...\n");

	while(1)
	{
		readycode = poll(clientfd_array,4096,-1);
